...
#These are fixed, and thus do not have to be changed ever
...
def one_hot_encode(X_train_hier, X_val_hier):
    """Take the heirarachy and one hot encode it"""
    
    ohe = sklearn.preprocessing.OneHotEncoder()
    
    # This ensures that all categories are in the one hot encoding
...
def create_dataset_multi(filenames, hierarchy, labels, BATCH_SIZE=64, SHUFFLE_BUFFER_SIZE = 1024):
    ...
    dataset_input = dataset_input.map(parse_function_multi, num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def create_dataset(filenames, labels, BATCH_SIZE = 64, SHUFFLE_BUFFER_SIZE = 1024):
    """Load and parse dataset.
    Args:
        filenames: list of image paths
        labels: numpy array of shape (BATCH_SIZE, N_LABELS)
...
def print_layer_trainable(conv_model):
    """"goes through each layer to make sure that the model has the correct training settings"""
